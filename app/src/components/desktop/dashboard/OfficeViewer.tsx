...
export function OfficeViewer({
    ...
    const isExecutable = useMemo(() => isExecutableFile(file.name, file.mime_type), [file.name, file.mime_type]);
    ...
    const isPowerPoint = ext === 'pptx' || ext === 'ppt' || ext === 'odp';

    const appName = isExcel ? 'Excel' : isPowerPoint ? 'PowerPoint' : 'Word';

    // Handle Open in External App (Microsoft Word / Excel / PowerPoint / System App - Lazy Download)
    const handleOpenExternally = async () => {
        if (isBlocked || openingExternal) {
    ...
    // Keyboard shortcuts (Enter -> Open in System App, Esc -> Close, Arrow keys -> Navigation)
    useEffect(() => {
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <div className={`absolute -top-24 -left-24 w-64 h-64 rounded-full blur-3xl opacity-20 pointer-events-none ${isBlocked ? 'bg-red-500' : isExcel ? 'bg-emerald-500' : isPowerPoint ? 'bg-amber-500' : 'bg-blue-500'}`} />
                    ...
                    <div className="flex items-center justify-center gap-2 mb-6 text-xs text-telegram-subtext">
                        <span className={`px-2.5 py-0.5 rounded-full font-bold uppercase ${
                            ...
                            {isBlocked ? (securityBlocked ? 'DISGUISED EXECUTABLE BINARY' : 'EXECUTABLE PROGRAM') : (ext || 'OFFICE')}
                        </span>
                        <span>•</span>
                    ...
                    {isBlocked && (
                        <div className="w-full bg-red-500/10 border border-red-500/25 rounded-2xl p-4 mb-6 text-left text-xs text-red-300 flex gap-3 items-start">
                            ...
                            <div className="leading-relaxed">
                                ...
                                For your security, disguised executable files cannot be opened directly from Telegram-Drive. Download the file to your computer to inspect and run it using your OS antivirus.
                            </div>
                        </div>
                    )}
                    ...
                    {/* Action 2: Open in System App / Microsoft Office (Only if non-blocked) */}
                    {!isBlocked && (
                        <button
                        ...
                        >
                            {openingExternal ? (
                                ...
                                <>
                                    ...
                                    <span>Open in Microsoft {appName} / System Reader</span>
                                </>
                            )}
                        </button>
                    )}
                    ...
                    <p className="text-xs text-telegram-subtext">
                        {isBlocked ? (
                            ...
                            <span>Click <strong className="text-white">Download</strong> to save directly to your PC, or <strong className="text-white">Open in {appName}</strong> to view in Microsoft Office.</span>
                        )}
                    </p>
                </div>
            </div>
